...
# Set data download directory (session only),  if you want to have  ebird data layers on a specific folder

# check the directory, for practicality make sure the directory is the species_layers folder in the data folder 
ebirdst_data_dir()

# here is an example 
# Sys.setenv(
#   EBIRDST_DATA_DIR =
#     ""C:/Users/jmunoz/data/species_layers")
# 
...
# Download example.
#here I am downloading some species to explore extent, note that I am using the 3 km layers. This is just to explore what is being downloaded 
...
# Here I am using a file with the species of interest to create a list, the list will be used to download all the species later on 
...
# ALTERNATIVELY you can create a list of species of interest, create your list here
...
# 1)a folder named "conservation_polygon" and include there the polygons for potential areas of conservation as a  shape file 
...
# Make sure that you change the naming of the columns to be readable to R
...
# Clean teh acad data set so that R reads it properly and select only colums of interest 
...
# ================================================================
# This creates the functions that we need later
# Run once
# ------------------------------------------------
...
estimate_pop_conservation_area_breeding <- function(species,
  ...
  abd <- load_raster(
    ...
    product = "abundance",
    period = "seasonal",
    metric = "max", # note that here we can use mean or max
    resolution = "3km")
...
#estimate_pop_conservation_area_breeding(species,conservation_polygon,ACAD_clean) # you can use this one too



# Here are some problems that we will encounter

# Species that have have small numbers and are very secretive
...
# I want to know the sum (total proportion ) of population that leaves in NA in both datasets
